...
st.write("Tickers ready:", len(tickers))
...
st.write("Momentum_12:",momentum_12.shape)
...
st.write("Rank_12:",ranked_12.shape)
...
st.write("Stocks bought in first month (90th pct):", buy_signal_90.iloc[0].sum())

buy_signal = ranked_12.gt(ranked_12.quantile(0.98, axis=1), axis=0)
st.write("Stocks bought in first month (98th pct):", buy_signal.iloc[0].sum())
...
st.write("Final Cumulative Return:", cumulative_return.iloc[-1])
st.write("Total Return %:", round((cumulative_return.iloc[-1]) * 100, 2), "%")
...
for n in [10, 20, 46]:
    ...
    st.write("Top", n, "stocks →", round((cumul-1)*100, 2), "% return")
...
st.write("Current month buys")
st.write(buy_signal.index[-1].date())
st.write(buy_signal.iloc[-1][buy_signal.iloc[-1]].index.tolist())
...
st.write("Stocks held (ranked as of", target_month.date(), "), sorted by realized return:")
st.write(actual_returns.sort_values())
st.write("Manual average:", actual_returns.mean())
...
st.write("Current holdings (ranked as of June 30):")
...
st.write("Number of stocks:", len(current_90pct))
st.write(current_90pct)
...
st.write(current_returns_so_far)
...
st.write("Performance since start of month, sorted worst to best:")
st.write(pct_change_so_far.sort_values())
